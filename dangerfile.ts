...
async function checkUiPackage() {
  ...
  for (const f of created_ui_index_files) {
    ...
    if (diff?.added.includes('import @safubit/appkit-core')) {
      fail(`${f} is importing @safubit/appkit-core, which is not allowed in ui package`)
    }
    ...
    if (diff?.added.includes('@safubit/appkit-ui/')) {
...
async function checkCorePackage() {
  ...
  for (const f of created_core_controllers) {
    ...
    if (diff?.added.includes('import @safubit/appkit-ui')) {
      fail(`${f} is importing @safubit/appkit-ui, which is not allowed in core package`)
    }
    ...
    if (diff?.added.includes('@safubit/appkit-core/')) {
      fail(`${f} should use relative imports instead of direct package access`)
    }
...
async function checkScaffoldHtmlPackage() {
  ...
  for (const f of created_scaffold_index_files) {
    ...
    if (
      diff?.added.includes('@safubit/appkit-core/') ||
      diff?.added.includes('@safubit/appkit-ui/') ||
      diff?.added.includes('@safubit/scaffold/')
    ) {
      fail(`${f} should use relative imports instead of direct package access`)
    }
  }
...
async function checkClientPackages() {
  ...
  for (const f of client_files) {
    ...
    if (diff?.added.includes("from '@safubit/appkit-core")) {
      fail(`${f} is not allowed to import from @safubit/appkit-core`)
    }

    if (diff?.added.includes("from '@safubit/appkit-ui")) {
      fail(`${f} is not allowed to import from @safubit/appkit-ui`)
    }
