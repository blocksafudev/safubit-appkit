...
import { setColorTheme, setThemeVariables } from '@safubit/appkit-ui'
import {
  ConstantsUtil,
  type CaipNetwork,
  type ChainNamespace,
...
import { CaipNetworksUtil, ErrorUtil } from '@safubit/appkit-utils'
import type { W3mFrameTypes } from '@safubit/appkit-wallet'
  ...
  private async initOrContinue() {
    if (!this.initPromise && !isInitialized && CoreHelperUtil.isClient()) {
      ...
      this.initPromise = new Promise<void>(async resolve => {
        await Promise.all([
          import('@safubit/appkit-ui'),
          import('@safubit/appkit-scaffold-ui/w3m-modal')
        ])
